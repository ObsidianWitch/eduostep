pub fn xor8sum<'a>(iterator: impl Iterator<Item = &'a u8>) -> u8 {
    return iterator.fold(0u8, |acc, b| acc ^ b);
}

pub fn fletcher8sum<'a>(iterator: impl Iterator<Item = &'a u8>) -> (u8, u8) {
    return iterator.fold((0u8, 0u8), |acc, b| {
...
pub fn crc16sum<'a>(iterator: impl Iterator<Item = &'a u8>) -> u16 {
    return iterator.fold(0u16, |mut crc, b| {
        crc ^= (*b as u16) << 8;
        for _ in 0..8 {
...
mod tests {
    ...
    fn checksum_manual() {
        ...
        assert!(xor8sum(data.iter()) == 3u8);
        assert!(fletcher8sum(data.iter()) == (26u8,183u8));
        assert!(crc16sum(data.iter()) == 0xda91);
    }
}
